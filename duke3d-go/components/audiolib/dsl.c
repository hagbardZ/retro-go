...
static uint8_t *_remainder_ptr = NULL;

IRAM_ATTR void audio_cb( void *udata, unsigned char *stream, int len )
{
	Uint8 *stptr = (Uint8 *)stream;
	int copysize;
	
	while (len > 0) {
        if (_remainder > 0) {
            copysize = min(len, _remainder);
            memcpy(stptr, _remainder_ptr, copysize);
            
            len -= copysize;
            stptr += copysize;
            _remainder -= copysize;
            _remainder_ptr += copysize;
        } else {
            _CallBackFunc();
            _remainder = _BufferSize;
            _remainder_ptr = (Uint8 *)(&_BufferStart[MV_MixPage * _BufferSize]);
        }
	}
}
