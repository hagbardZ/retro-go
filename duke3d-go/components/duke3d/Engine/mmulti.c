...
EXT_RAM_BSS_ATTR static gcomtype *gcom;
...
void dosendpackets(long other)
{
	...
	dacrc = getcrc((uint8_t *)gcom->buffer,(short)k);
...
void unstable_sendlogoff(void)
{
	...
	for(i=connecthead;i>=0;i=connectpoint2[i])
		if (i != myconnectindex)
			sendpacket(i,(uint8_t *)tempbuf,2L);
}
...
short unstable_getpacket (short *other, char *bufptr)
{
	...
	*other = gcom->other-1;
	...
	dacrc += (((unsigned short)gcom->buffer[messleng-1])<<8);
			if (dacrc != getcrc((uint8_t *)gcom->buffer,(short)(messleng-2)))        /* CRC check */
