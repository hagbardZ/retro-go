...
void cheatkeys(short snum)
{
    ...
    sb_snum = dukeSyncArray[snum].bits;
    p = &ps[snum];
    ...
    if( playing_old_demo && !(sb_snum&((15<<8)|(1<<12)|(1<<15)|(1<<16)|(1<<22)|(1<<19)|(1<<20)|(1<<21)|(1<<24)|(1<<25)|(1<<27)|(1<<28)|(1<<29)|(1<<30)|(1<<31))) )
		...
		((sb_snum && ( dukeSyncArray[snum].bits&(1<<17) ) == 0) && !playing_old_demo))
    {
		if(playing_old_demo)
...
void checksectors(short snum)
{
    ...
    if( ud.cashman && dukeSyncArray[snum].bits&(1<<29) )
        lotsofmoney(&sprite[p->i],2);

    if(p->newowner >= 0)
    {
        if( klabs(dukeSyncArray[snum].svel) > 768 || klabs(dukeSyncArray[snum].fvel) > 768 )
        {
            i = -1;
            goto CLEARCAMERAS;
        }
    ...
    if( !(dukeSyncArray[snum].bits&(1<<29)) && !(dukeSyncArray[snum].bits&(1<<31)))
        p->toggle_key_flag = 0;

    else if(!p->toggle_key_flag)
    {

        if( (dukeSyncArray[snum].bits&(1<<31)) )
        {
            if( p->newowner >= 0 )
            {
                i = -1;
        ...
        if( (dukeSyncArray[snum].bits&(1<<29)) == 0 ) return;
