...
EXT_RAM_BSS_ATTR struct weaponhit hittype[MAXSPRITES];
EXT_RAM_BSS_ATTR short spriteq[1024];
...
EXT_RAM_BSS_ATTR struct animwalltype animwall[MAXANIMWALLS];
short numanimwalls;
EXT_RAM_BSS_ATTR int32_t *animateptr[MAXANIMATES], animategoal[MAXANIMATES], animatevel[MAXANIMATES], animatecnt;
// int32_t oanimateval[MAXANIMATES];
short animatesect[MAXANIMATES];
EXT_RAM_BSS_ATTR int32_t msx[2048],msy[2048];
short cyclers[MAXCYCLERS][6],numcyclers;

EXT_RAM_BSS_ATTR char  fta_quotes[NUMOFFIRSTTIMEACTIVE][64];

EXT_RAM_BSS_ATTR uint8_t  tempbuf[2048];
EXT_RAM_BSS_ATTR uint8_t packbuf[576];
...
EXT_RAM_BSS_ATTR char level_names[44][33];
EXT_RAM_BSS_ATTR char level_file_names[44][128];
...
EXT_RAM_BSS_ATTR int32_t soundsiz[NUM_SOUNDS];

EXT_RAM_BSS_ATTR short soundps[NUM_SOUNDS],soundpe[NUM_SOUNDS],soundvo[NUM_SOUNDS];
EXT_RAM_BSS_ATTR uint8_t  soundm[NUM_SOUNDS],soundpr[NUM_SOUNDS];
EXT_RAM_BSS_ATTR char  sounds[NUM_SOUNDS][14];

short title_zoom;

fx_device device;

EXT_RAM_BSS_ATTR SAMPLE Sound[ NUM_SOUNDS ];
EXT_RAM_BSS_ATTR SOUNDOWNER SoundOwner[NUM_SOUNDS][4];

uint8_t  numplayersprites,earthquaketime;

int32_t fricxv,fricyv;
struct player_orig po[MAXPLAYERS];
EXT_RAM_BSS_ATTR struct player_struct ps[MAXPLAYERS];
struct user_defs ud;

uint8_t  pus, pub;
EXT_RAM_BSS_ATTR int32_t  syncstat;
EXT_RAM_BSS_ATTR uint8_t  syncval[MAXPLAYERS][MOVEFIFOSIZ];
int32_t syncvalhead[MAXPLAYERS], syncvaltail, syncvaltottail;

EXT_RAM_BSS_ATTR input inputfifo[MOVEFIFOSIZ][MAXPLAYERS], dukeSyncArray[MAXPLAYERS];
input loc;
EXT_RAM_BSS_ATTR input recsync[RECSYNCBUFSIZ];
...
EXT_RAM_BSS_ATTR int32_t script[MAXSCRIPTSIZE];
int32_t *scriptptr,*insptr,*labelcode,labelcnt;
EXT_RAM_BSS_ATTR int32_t *actorscrptr[MAXTILES];
int32_t *parsing_actor;
char  *label,*textptr,error,warning ;
uint8_t killit_flag;
uint8_t  *music_pointer;
EXT_RAM_BSS_ATTR uint8_t  actortype[MAXTILES];


uint8_t  display_mirror,typebuflen;
char typebuf[41];

EXT_RAM_BSS_ATTR char  music_fn[4][11][13];
...
int32_t myx, omyx, myy, omyy, myz, omyz;
int32_t myxvel, myyvel, myzvel;
...
EXT_RAM_BSS_ATTR int32_t myxbak[MOVEFIFOSIZ], myybak[MOVEFIFOSIZ], myzbak[MOVEFIFOSIZ];
EXT_RAM_BSS_ATTR int32_t myhorizbak[MOVEFIFOSIZ];
int32_t dukefriction = 0xcc00, show_shareware;

short myangbak[MOVEFIFOSIZ];
char  duke_myname[32] = "XDUKE";
int32_t  camerashitable,freezerhurtowner=0,lasermode;
// CTW - MODIFICATION
// uint8_t  networkmode = 255, movesperpacket = 1,gamequit = 0,playonten = 0,everyothertime;
uint8_t  networkmode = 255, movesperpacket = 1;
uint8_t gamequit = 0;
...
int32_t numclouds,clouds[128],cloudx[128],cloudy[128];
int32_t cloudtotalclock = 0,totalmemory = 0;
int32_t numinterpolations = 0, startofdynamicinterpolations = 0;
EXT_RAM_BSS_ATTR int32_t oldipos[MAXINTERPOLATIONS];
EXT_RAM_BSS_ATTR int32_t bakipos[MAXINTERPOLATIONS];
EXT_RAM_BSS_ATTR int32_t *curipos[MAXINTERPOLATIONS];
...
void Error (int errorType, char  *error, ...)
{
   ...
   RG_LOGI("Error: Entering with type=%d", errorType);
   SDL_Quit();
