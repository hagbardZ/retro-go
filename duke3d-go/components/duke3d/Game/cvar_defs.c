...
void CVARDEFS_Render()
{
    if(g_CV_DebugJoystick)
    {
        ...
        {
            sprintf(buf, "Joystick Axis%d: Raw: %d  Used:%"PRId32, i,_joystick_axis(i), CONTROL_GetFilteredAxisValue(i));
	...
	if(g_CV_DebugSound)
	{
		...
		sprintf(buf, "Active sounds: %"PRIu32, sounddebugActiveSounds);
		minitext(2, 10, buf, 23,10+16);
		
		sprintf(buf, "Allocate Calls: %"PRIu32, sounddebugAllocateSoundCalls);
		minitext(2, 18, buf, 23,10+16);

		sprintf(buf, "Deallocate Calls: %"PRId32, sounddebugDeallocateSoundCalls);
