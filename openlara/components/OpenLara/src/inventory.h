...
#if defined(_OS_PSP) || defined(_OS_3DS) || defined(_OS_GCW0) || defined(__RETROGO__)
...
struct Inventory {
    ...
    struct Item {
        ...
        void initLoadSlots(TR::Level *level) {
            ...
            if (level->isTitle()) {
                ...
                if (optLoadSlots.length <= 2) {
                    optLoadSlots.push(OptionItem( OptionItem::TYPE_TITLE, STR_EMPTY ));
                }
            }
        }
    ...
    static void loadTitleBG(Stream *stream, void *userData) {
        ...
        inv->titleTimer = (inv->game && inv->game->getLevel()->isTitle()) ? 0.0f : 3.0f;
    ...
    void init(bool playLogo, bool playVideo) {
        ...
        if (playLogo) {
            TR::Version version = game ? game->getLevel()->version : TR::VER_TR1;
            new Stream(TR::getGameLogo(version), loadLogo, this);
            return;
        }
        ...
        if (phaseRing == 0.0f || phaseRing == 1.0f) {
            ...
            if (active) {
                ...
                if (type == TR::Entity::INV_PASSPORT) {
                    items[contains(type)]->initLoadSlots(game->getLevel());
                }
    ...
    void controlPassport(Item *item, ControlKey key) {
        TR::Level *level = game->getLevel();
        int oldValue = item->value;
        int maxPage = 2;
        if (key == cLeft  && item->value > 0)       { item->value--; item->anim->dir = -1.0f; game->playSound(TR::SND_INV_PAGE); }
        if (key == cRight && item->value < maxPage) { item->value++; item->anim->dir =  1.0f; game->playSound(TR::SND_INV_PAGE); }

        if (item->value != oldValue) {
            if (level->isTitle() && item->value == 0) {
                item->initLoadSlots(level);
            }
            slot = 0;
            item->nextSlot(slot, -1);
        ...
        if (key == cAction && phaseChoose == 1.0f) {
            ...
            if (level->isTitle()) {
                switch (item->value) {
                    case 0 : { // load sub-menu
                        int optCount;
                        const OptionItem *opt = item->getOptions(optCount);
                        if (opt && slot != -1) {
                            opt = opt + slot;
                            if (opt->color >= 0) {
                    ...
                    case 1 : nextLevel = level->getStartId(); break;
                    case 2 : Core::quit(); break;
                }
            } else {
                if (item->value == 0) game->saveGame(id, true, false);
                if (item->value == 1) game->loadGame(0);
                if (item->value == 2) nextLevel = level->getTitleId();

                if (nextLevel != TR::LVL_MAX || item->value < 2) {
                    item->anim->dir = -1.0f;
                    item->value = -100;
                    toggle();
                }
    ...
    void controlItem(Item *item, ControlKey key) {
        ...
        if (item->type == TR::Entity::INV_PASSPORT) {
            controlPassport(item, key);
            if (game->getLevel()->isTitle() && item->value == 0) {
                // Allow Title Screen LOAD page to fall through to item->control
            } else {
                return;
            }
        }
    ...
    void renderPassport(Item *item) {
        if (item->anim->dir != 0.0f) return; // check for animation

        StringID str;
        int maxPage = 2;

        if (game->getLevel()->isTitle()) {
            if (item->value == 0) str = STR_LOAD_GAME;
            if (item->value == 1) str = STR_START_GAME;
            if (item->value == 2) str = STR_EXIT_GAME;
        } else {
            if (item->value == 0) str = STR_SAVEGAME;
        ...
        UI::textOut(vec2(eye, UI::height - 48), str, UI::aCenter, UI::width);
        int tw = UI::getTextSize(STR[str]).x;

        if (item->value > 0)       UI::specOut(vec2((UI::width - tw) * 0.5f - 32.0f + eye, UI::height - 48), 108);
        if (item->value < maxPage) UI::specOut(vec2((UI::width + tw) * 0.5f + 16.0f + eye, UI::height - 48), 109);

        // Sub-menu for Load Game on Title Screen
        if (game->getLevel()->isTitle() && item->value == 0) {
            if (slot == -1) {
                //
            } else
    ...
    void renderItemText(Item *item) {
        ...
        renderItemCount(item, vec2(UI::width / 2 - 160, UI::height - 96), 320);
    ...
    // show health bar in inventory when selector is over medikit
        ...
        if (phaseChoose == 1.0f) {
            switch (item->type) {
                ...
                case TR::Entity::INV_STOPWATCH :
                case TR::Entity::INV_MAP       :
                    UI::textOut(vec2(0, UI::height * 0.5f), STR_EMPTY, UI::aCenter, UI::width);
                    break;
                default : ;
            }
        }
    ...
    void renderBackground(int view) {
        ...
        if (background[0] && background[0]->origWidth / background[0]->origHeight == 2) // PSX images aspect correction
            sy = (UI::height / (UI::height * 4.0f / 3.0f)) * ((float)background[0]->origWidth / (float)background[0]->origHeight);
    ...
    void renderUI() {
        ...
        if (page == PAGE_SAVEGAME) {
            UI::renderBar(CTEX_OPTION, vec2(UI::width / 2 - 120, UI::height * 0.5f - 14), vec2(240, LINE_HEIGHT - 6), 1.0f, 0x802288FF, 0, 0, 0);
            UI::textOut(vec2(0, UI::height * 0.5f), pageTitle[page], UI::aCenter, UI::width);
            UI::renderBar(CTEX_OPTION, vec2(-slot * 48 + UI::width / 2, UI::height * 0.5f + 24 - 16), vec2(48, 18), 1.0f, 0xFFD8377C, 0);
            UI::textOut(vec2(UI::width / 2 - 48, UI::height * 0.5f + 24), STR_YES, UI::aCenter, 48);
            UI::textOut(vec2(UI::width / 2, UI::height * 0.5f + 24), STR_NO, UI::aCenter, 48);
            return;
        }
        ...
        if (canFlipPage(1)) {
            UI::textOut(vec2(16, UI::height - 16), "]", UI::aLeft, UI::width);
            UI::textOut(vec2( 0, UI::height - 16), "]", UI::aRight, UI::width - 20);
        }
    ...
    // inventory controls help
        if (page == targetPage && Input::touchTimerVis <= 0.0f) {
            ...
            sprintf(buf, "A - Select");
            UI::textOut(vec2(eye + dx, UI::height - 24), buf, UI::aLeft, UI::width);
            if (chosen) {
                sprintf(buf, "B - Go Back");
                UI::textOut(vec2(eye, UI::height - 24), buf, UI::aRight, UI::width - dx);
            }
        }
