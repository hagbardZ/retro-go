...
// level
    struct Geometry {
        ...
        MeshRange* getNextRange(int vStart, int iCount, uint16 tile, uint16 clut) {
            ...
            if (!range || range->iCount) {
                if (count >= (int)COUNT(ranges)) {
                    #ifdef _DEBUG
                        LOG("MeshBuilder: too many ranges! (%d)\n", count);
                    #endif
                    return NULL;
                }
                range = &ranges[count++];
    ...
    struct Dynamic {
        ...
        void init(int count) {
            this->count = count;
            this->faces = count ? (uint16*)OL_ALLOC(count * sizeof(uint16)) : NULL;
            if (this->faces) memset(this->faces, 0, count * sizeof(uint16));
        }
...
// procedured
    ...
    // allocate room geometry ranges
        rooms = (RoomRange*)OL_ALLOC(level->roomsCount * sizeof(RoomRange));
        if (rooms) memset(rooms, 0, level->roomsCount * sizeof(RoomRange));
    ...
    // get models info
        ...
        models = (ModelRange*)OL_ALLOC(level->modelsCount * sizeof(ModelRange));
        if (models) memset(models, 0, level->modelsCount * sizeof(ModelRange));
    ...
    // make meshes buffer (single vertex buffer object for all geometry & sprites on level)
        Index  *indices  = (Index*)OL_ALLOC(iCount * sizeof(Index));
        if (indices) memset(indices, 0, iCount * sizeof(Index));
        Vertex *vertices = (Vertex*)OL_ALLOC(vCount * sizeof(Vertex));
        if (vertices) memset(vertices, 0, vCount * sizeof(Vertex));
        iCount = vCount = 0;
        int aCount = 0;
