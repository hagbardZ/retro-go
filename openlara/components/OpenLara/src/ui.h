...
namespace UI {
    ...
    void renderSubs() {
        ...
        if (subsTime > 0.0f) {
            char buf[256];
            const char *src = STR[subsStr] + subsPos;
            int len = 0;
            int lastSpace = -1;
            int linePixels = 0;

            while (len < 255 && src[len] && src[len] != '[') {
                char c = src[len];
                buf[len] = c;
                if (c == ' ') lastSpace = len;
                
                linePixels += char_width[charRemap(c)] + 1;

                if (linePixels > 320 && lastSpace != -1) {
                    buf[lastSpace] = '@';
                    linePixels = 0;
                    for (int k = lastSpace + 1; k <= len; k++)
                        linePixels += char_width[charRemap(buf[k])] + 1;
                    lastSpace = -1;
                }
                len++;
            }
            ...
            textOut(vec2(16, height - 32) + vec2(1, 1), buf, aCenterV, width - 32, 255, UI::SHADE_GRAY, true);
            textOut(vec2(16, height - 32), buf, aCenterV, width - 32, 255, UI::SHADE_GRAY);
        }
    }
