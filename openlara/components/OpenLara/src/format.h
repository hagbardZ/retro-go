   ...
   // common
        ...
        // rooms
            for (int i = 0; i < roomsCount; i++) {
                Room &r = rooms[i];
                OL_FREE(r.data.vertices);
                OL_FREE(r.data.faces);
                OL_FREE(r.data.sprites);
                OL_FREE(r.portals);
                OL_FREE(r.sectors);
                OL_FREE(r.lights);
                OL_FREE(r.meshes);
            ...
            OL_FREE(frameData);
            OL_FREE(models);
            OL_FREE(staticMeshes);
            OL_FREE(objectTextures);
            OL_FREE(objectTexturesData);
            OL_FREE(roomTextures);
            OL_FREE(roomTexturesData);
            OL_FREE(itemTextures);
            OL_FREE(itemTexturesData);
            OL_FREE(spriteTextures);
            OL_FREE(spriteSequences);
            OL_FREE(spriteTexturesData);
            OL_FREE(cameras);
            OL_FREE(flybyCameras);
            OL_FREE(soundSources);
            OL_FREE(boxes);
            OL_FREE(overlaps);
            for (int i = 0; i < 2; i++) {
                OL_FREE(zones[i].ground1);
                OL_FREE(zones[i].ground2);
                OL_FREE(zones[i].ground3);
                OL_FREE(zones[i].ground4);
                OL_FREE(zones[i].fly);
            }
            for (int i = 0; i < animTexturesCount; i++)
                OL_FREE(animTextures[i].textures);
            OL_FREE(animTextures);
            OL_FREE(entities);
            OL_FREE(palette);
            OL_FREE(palette32);
            OL_FREE(cluts);
            ...
            OL_FREE(tiles32);
            OL_FREE(tilesMisc);
            OL_FREE(cameraFrames);
            OL_FREE(AIObjects);
            OL_FREE(demoData);
            OL_FREE(soundsMap);
            OL_FREE(soundsInfo);
            OL_FREE(soundData);
            OL_FREE(soundOffsets);
            OL_FREE(soundSize);
        ...
        void loadTR1_PSX (Stream &stream) {
            ...
            // sound offsets
                ...
                soundOffsets = (uint32*)OL_ALLOC(soundOffsetsCount * sizeof(uint32));
                if (soundOffsets) memset(soundOffsets, 0, soundOffsetsCount * sizeof(uint32));
                soundSize = (uint32*)OL_ALLOC(soundOffsetsCount * sizeof(uint32));
                if (soundSize) memset(soundSize, 0, soundOffsetsCount * sizeof(uint32));
                soundDataSize = 0;
            ...
            #if defined(_GAPI_SW)
                // Convert PS1 colors (1555 BGR) to native RGB565
                for (int i = 0; i < clutsCount * 16; i++) {
                    uint16 c = ((uint16*)cluts)[i];
                    uint8 b = (c & 0x1F) << 3;
                    uint8 g = ((c >> 5) & 0x1F) << 3;
                    uint8 r = ((c >> 10) & 0x1F) << 3;
                    ((uint16*)cluts)[i] = CONV_COLOR(r, g, b);
                }

                // Copy first 256 colors to global palette for UI and colored geometry
                for (int i = 0; i < 256; i++) {
                    GAPI::swPaletteColor[i] = ((GAPI::ColorSW*)cluts)[i];
                }

                tiles8 = (Tile8*)OL_ALLOC(tilesCount * sizeof(Tile8));
                for (int i = 0; i < tilesCount; i++) {
                    for (int j = 0; j < 256 * 256 / 2; j++) {
                        uint8 b = ((uint8*)tiles4[i].index)[j];
                        tiles8[i].index[j * 2 + 0] = b & 0x0F;
                        tiles8[i].index[j * 2 + 1] = b >> 4;
                    }
                }
            #endif
        ...
        // get file name without extension
            ...
            char *name = (char*)OL_ALLOC((len + 1) * sizeof(char));
            if (name) memset(name, 0, (len + 1) * sizeof(char));
            ...
            if (Stream::existsContent(name)) {
                ...
                for (int i = int(len) - 1; i >= 0; i--)
                    if (name[i] == '/' || name[i] == '\\') {
                        char *newName = (char*)OL_ALLOC((len + 11 + 1) * sizeof(char));
                        if (newName) memset(newName, 0, (len + 11 + 1) * sizeof(char));
                        name[i] = 0;
                        strcpy(newName, name);
        ...
        void loadTR2_PC (Stream &stream) {
            ...
            // Skip tiles16 (Software rasterizer uses tiles8)
            stream.seek(tilesCount * 256 * 256 * 2);
        ...
        // skip code modules
            ...
            if (roomTexturesCount) {
                roomTextures = (TextureInfo*)OL_ALLOC(roomTexturesCount * sizeof(TextureInfo));
                if (roomTextures) memset(roomTextures, 0, roomTexturesCount * sizeof(TextureInfo));
        ...
        // tiles32
            ...
            tiles32 = (Tile32*)OL_ALLOC(sizeD);
            if (tiles32) memset(tiles32, 0, sizeD);
        ...
        // tiles16
            ...
            tilesMisc = (Tile32*)OL_ALLOC(sizeD);
            if (tilesMisc) memset(tilesMisc, 0, sizeD);
            ...
            stream.read(sizeD);
            stream.read(dataC, stream.read(sizeC));
            dataD = (uint8*)OL_ALLOC(sizeD);
            if (dataD) memset(dataD, 0, sizeD);
            ...
            soundData = (uint8*)OL_ALLOC(soundDataSize);
            if (soundData) memset(soundData, 0, soundDataSize);
            soundDataSize = 0;
        ...
        void readDataArrays(Stream &stream) {
            ...
            rooms = stream.read(roomsCount) ? (Room*)OL_ALLOC(roomsCount * sizeof(Room)) : NULL;
            if (rooms) memset(rooms, 0, roomsCount * sizeof(Room));
            for (int i = 0; i < roomsCount; i++) {
                readRoom(stream, i);
            }
        ...
        void readAnims(Stream &stream) {
            ...
            anims = animsCount ? (Animation*)OL_ALLOC(animsCount * sizeof(Animation)) : NULL;
            if (anims) memset(anims, 0, animsCount * sizeof(Animation));
        ...
        void readModels(Stream &stream) {
            models = stream.read(modelsCount) ? (Model*)OL_ALLOC(modelsCount * sizeof(Model)) : NULL;
            if (models) memset(models, 0, modelsCount * sizeof(Model));
        ...
        void readBoxes(Stream &stream) {
            boxes = stream.read(boxesCount) ? (Box*)OL_ALLOC(boxesCount * sizeof(Box)) : NULL;
            if (boxes) memset(boxes, 0, boxesCount * sizeof(Box));
        ...
        void readSoundMap(Stream &stream) {
            ...
            soundsInfo = (stream.read(soundsInfoCount) > 0) ? (SoundInfo*)OL_ALLOC(soundsInfoCount * sizeof(SoundInfo)) : NULL;
            if (soundsInfo) memset(soundsInfo, 0, soundsInfoCount * sizeof(SoundInfo));
                ...
                */
                ...
                // SAT
                    ...
                    case SAT_ROOMTINF :
                        ...
                        roomTextures = roomTexturesCount ? (TextureInfo*)OL_ALLOC(roomTexturesCount * sizeof(TextureInfo)) : NULL;
                        if (roomTextures) memset(roomTextures, 0, roomTexturesCount * sizeof(TextureInfo));
                        for (int i = 0; i < roomTexturesCount; i++)
                            readObjectTex(stream, roomTextures[i], TEX_TYPE_ROOM);
                        break;
                    case SAT_ROOMTQTR : {
                        ...
                        roomTexturesData = roomTexturesDataSize ? (uint8*)OL_ALLOC(roomTexturesDataSize * sizeof(uint8)) : NULL;
                        if (roomTexturesData) memset(roomTexturesData, 0, roomTexturesDataSize * sizeof(uint8));
...
/*
                    ...
                    */
                    ...
                    case SAT_ROOMTSUB :
                        ...
                        */
                        ...
                        tsub = (uint8*)OL_ALLOC(tsubCount * sizeof(uint8));
                        if (tsub) memset(tsub, 0, tsubCount * sizeof(uint8));
                    ...
                    case SAT_ROOMDATA :
                        ...
                        rooms = (Room*)OL_ALLOC(roomsCount * sizeof(Room));
                        if (rooms) memset(rooms, 0, sizeof(Room) * roomsCount);
                        break;
                    case SAT_ROOMNUMB :
                        ASSERTV(stream.readBE32() == 0x00000000);
                    ...
                    case SAT_MESHSIZE : {
                        ...
                        if (flag == 0x00000014) {
                            ...
                            room->meshes = room->meshesCount ? (Room::Mesh*)OL_ALLOC(room->meshesCount * sizeof(Room::Mesh)) : NULL;
                            if (room->meshes) memset(room->meshes, 0, room->meshesCount * sizeof(Room::Mesh));
                        ...
                        data.vertices = (Room::Data::Vertex*)OL_ALLOC(data.vCount * sizeof(Room::Data::Vertex));
                        if (data.vertices) memset(data.vertices, 0, data.vCount * sizeof(Room::Data::Vertex));
                        ...
                        data.faces   = (Face*)OL_ALLOC(data.fCount * sizeof(Face));
                        if (data.faces) memset(data.faces, 0, data.fCount * sizeof(Face));
                        data.sprites = (Room::Data::Sprite*)OL_ALLOC(data.fCount * sizeof(Room::Data::Sprite));
                        if (data.sprites) memset(data.sprites, 0, data.fCount * sizeof(Room::Data::Sprite));
                    ...
                    case SAT_DOORDATA : {
                        ...
                        room->portals = (Room::Portal*)OL_ALLOC(room->portalsCount * sizeof(Room::Portal));
                        if (room->portals) memset(room->portals, 0, room->portalsCount * sizeof(Room::Portal));
                    ...
                    case SAT_FLOORSIZ : {
                        ...
                        room->sectors = count ? (Room::Sector*)OL_ALLOC(count * sizeof(Room::Sector)) : NULL;
                        if (room->sectors) memset(room->sectors, 0, count * sizeof(Room::Sector));
                    ...
                    case SAT_FLORDATA :
                        ...
                        floors = (FloorData*)OL_ALLOC(floorsCount * sizeof(FloorData));
                        if (floors) memset(floors, 0, floorsCount * sizeof(FloorData));
                    ...
                    case SAT_LIGHTSIZ : {
                        ...
                        room->lights = room->lightsCount ? (Room::Light*)OL_ALLOC(room->lightsCount * sizeof(Room::Light)) : NULL;
                        if (room->lights) memset(room->lights, 0, room->lightsCount * sizeof(Room::Light));
                    ...
                    case SAT_CAMERAS_ :
                        ...
                        cameras = camerasCount ? (Camera*)OL_ALLOC(camerasCount * sizeof(Camera)) : NULL;
                        if (cameras) memset(cameras, 0, camerasCount * sizeof(Camera));
                    ...
                    case SAT_SOUNDFX_ : {
                        ...
                        soundSources = soundSourcesCount ? (SoundSource*)OL_ALLOC(soundSourcesCount * sizeof(SoundSource)) : NULL;
                        if (soundSources) memset(soundSources, 0, soundSourcesCount * sizeof(SoundSource));
                    ...
                    case SAT_BOXES___ :
                        ...
                        boxes = boxesCount ? (Box*)OL_ALLOC(boxesCount * sizeof(Box)) : NULL;
                        if (boxes) memset(boxes, 0, boxesCount * sizeof(Box));
                        for (int i = 0; i < boxesCount; i++) {
                            Box &b = boxes[i];
                    ...
                    case SAT_OVERLAPS :
                        ...
                        overlaps = overlapsCount ? (Overlap*)OL_ALLOC(overlapsCount * sizeof(Overlap)) : NULL;
                        if (overlaps) memset(overlaps, 0, overlapsCount * sizeof(Overlap));
                    ...
                    case SAT_FLY_ZONE : {
                        ...
                        int32 count = stream.readBE32();
                        *ptr = count ? (uint16*)OL_ALLOC(count * sizeof(uint16)) : NULL;
                        if (*ptr) memset(*ptr, 0, count * sizeof(uint16));
                        for (int i = 0; i < count; i++)
                    ...
                    case SAT_ARANGES_ : {
                        ...
                        animTextures = (AnimTexture*)OL_ALLOC(animTexturesCount * sizeof(AnimTexture));
                        if (animTextures) memset(animTextures, 0, animTexturesCount * sizeof(AnimTexture));
                        for (int i = 0; i < animTexturesCount; i++) {
                            AnimTexture &animTex = animTextures[i];
                            uint32 first = stream.readBE32();
                            uint32 last  = stream.readBE32();
                            animTex.count    = last - first + 1;
                            animTex.textures = (uint16*)OL_ALLOC(animTex.count * sizeof(uint16));
                            if (animTex.textures) memset(animTex.textures, 0, animTex.count * sizeof(uint16));
                    ...
                    case SAT_ITEMDATA : {
                        ...
                        entities = (Entity*)OL_ALLOC(entitiesCount * sizeof(Entity));
                        if (entities) memset(entities, 0, entitiesCount * sizeof(Entity));
                ...
                // SAD
                    ...
                    case SAD_CHANGES_ :
                        ...
                        states = statesCount ? (AnimState*)OL_ALLOC(statesCount * sizeof(AnimState)) : NULL;
                        if (states) memset(states, 0, statesCount * sizeof(AnimState));
                    ...
                    case SAD_RANGES_z :
                        ...
                        ranges = rangesCount ? (AnimRange*)OL_ALLOC(rangesCount * sizeof(AnimRange)) : NULL;
                        if (ranges) memset(ranges, 0, rangesCount * sizeof(AnimRange));
                    ...
                    case SAD_COMMANDS :
                        ...
                        commands = commandsCount ? (int16*)OL_ALLOC(commandsCount * sizeof(int16)) : NULL;
                        if (commands) memset(commands, 0, commandsCount * sizeof(int16));
                    ...
                    case SAD_ANIBONES :
                        ...
                        nodesData = nodesDataSize ? (uint32*)OL_ALLOC(nodesDataSize * sizeof(uint32)) : NULL;
                        if (nodesData) memset(nodesData, 0, nodesDataSize * sizeof(uint32));
                    ...
                    case SAD_ANIMOBJ_ :
                        ...
                        models = modelsCount ? (Model*)OL_ALLOC(modelsCount * sizeof(Model)) : NULL;
                        if (models) memset(models, 0, modelsCount * sizeof(Model));
                        for (int i = 0; i < modelsCount; i++) {
                    ...
                    case SAD_STATOBJ_ :
                        ...
                        staticMeshes = staticMeshesCount ? (StaticMesh*)OL_ALLOC(staticMeshesCount * sizeof(StaticMesh)) : NULL;
                        if (staticMeshes) memset(staticMeshes, 0, staticMeshesCount * sizeof(StaticMesh));
                    ...
                    case SAD_FRAMES__ :
                        ...
                        frameData = frameDataSize ? (uint16*)OL_ALLOC(frameDataSize * sizeof(uint16)) : NULL;
                        if (frameData) memset(frameData, 0, frameDataSize * sizeof(uint16));
                    ...
                    case SAD_MESHPTRS :
                        ...
                        meshOffsets = meshOffsetsCount ? (int32*)OL_ALLOC(meshOffsetsCount * sizeof(int32)) : NULL;
                        if (meshOffsets) memset(meshOffsets, 0, meshOffsetsCount * sizeof(int32));
                    ...
                    case SAD_MESHDATA :
                        ...
                        meshData = meshDataSize ? (uint16*)OL_ALLOC(meshDataSize * sizeof(uint16)) : NULL;
                        if (meshData) memset(meshData, 0, meshDataSize * sizeof(uint16));
                    ...
                    case SAD_OTEXTINF :
                        ...
                        objectTextures = objectTexturesCount ? (TextureInfo*)OL_ALLOC(objectTexturesCount * 5 * sizeof(TextureInfo)) : NULL;
                        if (objectTextures) memset(objectTextures, 0, objectTexturesCount * 5 * sizeof(TextureInfo));
                    ...
                    case SAD_OTEXTDAT : {
                        ...
                        objectTexturesData = objectTexturesDataSize ? (uint8*)OL_ALLOC(objectTexturesDataSize * sizeof(uint8)) : NULL;
                        if (objectTexturesData) memset(objectTexturesData, 0, objectTexturesDataSize * sizeof(uint8));
                    ...
                    case SAD_ITEXTINF : {
                        ...
                        itemTextures = itemTexturesCount ? (TextureInfo*)OL_ALLOC(itemTexturesCount * 5 * sizeof(TextureInfo)) : NULL;
                        if (itemTextures) memset(itemTextures, 0, itemTexturesCount * 5 * sizeof(TextureInfo));
                    ...
                    case SAD_ITEXTDAT : {
                        ...
                        itemTexturesData = itemTexturesDataSize ? (uint8*)OL_ALLOC(itemTexturesDataSize * sizeof(uint8)) : NULL;
                        if (itemTexturesData) memset(itemTexturesData, 0, itemTexturesDataSize * sizeof(uint8));
                ...
                // SPR
                    ...
                    case SPR_SPRITINF : {
                        ...
                        spriteTextures = spriteTexturesCount ? (TextureInfo*)OL_ALLOC(spriteTexturesCount * sizeof(TextureInfo)) : NULL;
                        if (spriteTextures) memset(spriteTextures, 0, spriteTexturesCount * sizeof(TextureInfo));
                    ...
                    case SPR_SPRITDAT : {
                        ...
                        spriteTexturesData = spriteTexturesDataSize ? (uint8*)OL_ALLOC(spriteTexturesDataSize * sizeof(uint8)) : NULL;
                        if (spriteTexturesData) memset(spriteTexturesData, 0, spriteTexturesDataSize * sizeof(uint8));
                    ...
                    case SPR_OBJECTS_ : {
                        ...
                        spriteSequences = spriteSequencesCount ? (SpriteSequence*)OL_ALLOC(spriteSequencesCount * sizeof(SpriteSequence)) : NULL;
                        if (spriteSequences) memset(spriteSequences, 0, spriteSequencesCount * sizeof(SpriteSequence));
                ...
                // SND
                    case SND_SAMPLUT_ : {
                        ...
                        soundsMap = (int16*)OL_ALLOC(count * sizeof(int16));
                        if (soundsMap) memset(soundsMap, 0, count * sizeof(int16));
                        for (int i = 0; i < count; i++)
                    ...
                    case SND_SAMPINFS : {
                        ...
                        soundsInfo = soundsInfoCount ? (SoundInfo*)OL_ALLOC(soundsInfoCount * sizeof(SoundInfo)) : NULL;
                        if (soundsInfo) memset(soundsInfo, 0, soundsInfoCount * sizeof(SoundInfo));
                        for (int i = 0; i < soundsInfoCount; i++) {
                            SoundInfo &s = soundsInfo[i];
        ...
        void readCIN(Stream &stream) {
            ...
            cameraFrames = cameraFramesCount ? (CameraFrame*)OL_ALLOC(cameraFramesCount * sizeof(CameraFrame)) : NULL;
            if (cameraFrames) memset(cameraFrames, 0, cameraFramesCount * sizeof(CameraFrame));
            stream.raw(cameraFrames, cameraFramesCount * 16);
        }

        void appendObjectTex(TextureInfo *&objTex, int32 &count) {
            if (!objTex) return;
            TextureInfo *newObjectTextures = (TextureInfo*)OL_ALLOC((objectTexturesCount + count) * sizeof(TextureInfo));
            if (newObjectTextures) {
        ...
        void readSamples(Stream &stream) {
            soundDataSize = stream.size;
            soundData = (uint8*)OL_ALLOC_NP(soundDataSize);
            if (soundData) {
                memset(soundData, 0, soundDataSize);
                stream.read(soundData, soundDataSize);
            } else {
                LOG("! not enough RAM for sound effects (%d bytes needed)\n", soundDataSize);
                return;
            }
...
//                        ASSERT(f.colored || f.flags.texture < objectTexturesCount);
                        ...
                        if (t.type != TEX_TYPE_OBJECT) {
                            if (!dupObjTex) {
                                dupObjTex = (TextureInfo*)OL_ALLOC(256 * sizeof(TextureInfo));
                                if (dupObjTex) memset(dupObjTex, 0, 256 * sizeof(TextureInfo));
                            }
        ...
        // room data
            ...
            // pre-calculate number of vertices, triangles and rectangles for TR3 PSX format
                ...
                d.faces    = d.fCount ? (Face*)OL_ALLOC(d.fCount * sizeof(Face)) : NULL;
                if (d.faces) memset(d.faces, 0, d.fCount * sizeof(Face));
                d.vertices = d.vCount ? (Room::Data::Vertex*)OL_ALLOC(d.vCount * sizeof(Room::Data::Vertex)) : NULL;
                if (d.vertices) memset(d.vertices, 0, d.vCount * sizeof(Room::Data::Vertex));

                d.vCount = d.fCount = 0;
            } else {
                d.vertices = stream.read(d.vCount) ? (Room::Data::Vertex*)OL_ALLOC(d.vCount * sizeof(Room::Data::Vertex)) : NULL;
                if (d.vertices) memset(d.vertices, 0, d.vCount * sizeof(Room::Data::Vertex));
            }
        ...
        // sectors
            ...
            r.sectors = (r.zSectors * r.xSectors > 0) ? (Room::Sector*)OL_ALLOC(r.zSectors * r.xSectors * sizeof(Room::Sector)) : NULL;
            if (r.sectors) memset(r.sectors, 0, r.zSectors * r.xSectors * sizeof(Room::Sector));
        ...
        // lights
            r.lights = stream.read(r.lightsCount) ? (Room::Light*)OL_ALLOC(r.lightsCount * sizeof(Room::Light)) : NULL;
            if (r.lights) memset(r.lights, 0, r.lightsCount * sizeof(Room::Light));
        ...
        // meshes
            ...
            r.meshes = r.meshesCount ? (Room::Mesh*)OL_ALLOC(r.meshesCount * sizeof(Room::Mesh)) : NULL;
            if (r.meshes) memset(r.meshes, 0, r.meshesCount * sizeof(Room::Mesh));
        ...
        void initMesh(int mIndex, Entity::Type type = Entity::NONE) {
            ...
            switch (version) {
                case VER_TR1_SAT : {
                    mesh.vertices = (Mesh::Vertex*)OL_ALLOC(mesh.vCount * sizeof(Mesh::Vertex));
                    if (mesh.vertices) memset(mesh.vertices, 0, mesh.vCount * sizeof(Mesh::Vertex));
                    ...
                    mesh.faces = (Face*)OL_ALLOC(mesh.fCount * sizeof(Face));
                    if (mesh.faces) memset(mesh.faces, 0, mesh.fCount * sizeof(Face));
                ...
                case VER_TR5_PC : {
                    ...
                    mesh.faces  = mesh.fCount ? (Face*)OL_ALLOC(mesh.fCount * sizeof(Face)) : NULL;
                    if (mesh.faces) memset(mesh.faces, 0, mesh.fCount * sizeof(Face));
        ...
        void readObjectTex(Stream &stream) {
            objectTextures = stream.read(objectTexturesCount) ? (TextureInfo*)OL_ALLOC(objectTexturesCount * sizeof(TextureInfo)) : NULL;
            if (objectTextures) memset(objectTextures, 0, objectTexturesCount * sizeof(TextureInfo));
            for (int i = 0; i < objectTexturesCount; i++) {
        ...
        void readSpriteTex(Stream &stream) {
            spriteTextures = stream.read(spriteTexturesCount) ? (TextureInfo*)OL_ALLOC(spriteTexturesCount * sizeof(TextureInfo)) : NULL;
            if (spriteTextures) memset(spriteTextures, 0, spriteTexturesCount * sizeof(TextureInfo));
            for (int i = 0; i < spriteTexturesCount; i++)
                readSpriteTex(stream, spriteTextures[i]);

            spriteSequences = stream.read(spriteSequencesCount) ? (SpriteSequence*)OL_ALLOC(spriteSequencesCount * sizeof(SpriteSequence)) : NULL;
            if (spriteSequences) memset(spriteSequences, 0, spriteSequencesCount * sizeof(SpriteSequence));
            for (int i = 0; i < spriteSequencesCount; i++) {
                SpriteSequence &s = spriteSequences[i];
                uint16 type;
        ...
        void readAnimTex(Stream &stream) {
            ...
            if (animTexBlockSize) {
                uint16 *animTexBlock = (uint16*)OL_ALLOC(animTexBlockSize * sizeof(uint16));
                if (animTexBlock) {
                    ...
                    animTextures = animTexturesCount ? (AnimTexture*)OL_ALLOC(animTexturesCount * sizeof(AnimTexture)) : NULL;
                    if (animTextures) memset(animTextures, 0, animTexturesCount * sizeof(AnimTexture));
