...
namespace StrUtils {
    ...
    char* copy(const char *str) {
        ...
        char *res = (char*)OL_ALLOC(strlen(str) + 1);
        strcpy(res, str);
        return res;
    }
...
struct Array {
    ...
    void reserve(int capacity) {
        ...
        if (items)
            items = (T*)OL_REALLOC(items, capacity * sizeof(T));
        else
            items = (T*)OL_ALLOC(capacity * sizeof(T));
    }
...
private:
    ...
    void openFile() {
        ...
        if (!f) {
                ...
                osDownload(this);
            #else
                LOG("error loading file \"%s\" (tried \"%s\")\n", name, path);
                if (callback) {
                    callback(NULL, userData);
                    delete this;
                } else {
                    // Do nothing, let caller handle failed stream (e.g. stream->size == -1)
                }
            #endif
        } else {
            //LOG("opened file \"%s\"\n", path);
...
//dunno if this is used (?)
    ...
    #if defined(_OS_WIN) || defined(_OS_XBOX)
        for (int i = 0; str[i]; i++)
            if (str[i] == '/')
                str[i] = '\\';
    #else
    ...
    __attribute__((noinline)) static void normalizePath(char *path) {
        fixBackslash(path);
        if (exists(path)) return;

    #ifdef __RETROGO__
        // Get the requested filename without any folder prefixes
        ...
        // Extension-based router for maximum speed
        if (ext && strcasecmp(ext, ".wav") == 0) {
            sprintf(temp, "%saudio/1/%s", contentDir, name);
            if (exists(temp)) { strcpy(path, temp); return; }
            sprintf(temp, "%saudio/%s", contentDir, name);
            if (exists(temp)) { strcpy(path, temp); return; }
        } else if (ext && (strcasecmp(ext, ".phd") == 0 || strcasecmp(ext, ".psx") == 0)) {
            sprintf(temp, "%sdata/%s", contentDir, name);
            if (exists(temp)) { strcpy(path, temp); return; }
            sprintf(temp, "%sDATA/%s", contentDir, name);
            if (exists(temp)) { strcpy(path, temp); return; }
        } else if (ext && (strcasecmp(ext, ".png") == 0 || strcasecmp(ext, ".pcx") == 0)) {
            char base[128];
            ...
            const char *exts[] = { ".png", ".pcx" };
            const char *dirs[] = { "pix/", "PIX/", "data/", "DATA/" };

            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
                    sprintf(temp, "%s%s%s%s", contentDir, dirs[j], base, exts[i]);
                    if (exists(temp)) { strcpy(path, temp); return; }
                }
            }
        }

        // Generic fallback (only one check!)
        sprintf(temp, "%s%s", contentDir, name);
        if (exists(temp)) { strcpy(path, temp); return; }
    #else
        // Get the requested filename without any folder prefixes
        const char *name = strrchr(path, '/');
        ...
        // Standard Tomb Raider PC subfolders
        const char *dirs[] = {
            "PIX/", "pix/",
            "DATA/", "data/",
            "AUDIO/1/", "audio/1/",
        ...
        for (int i = 0; i < 11; i++) { // 11 is the number of dirs
            // Try 1: Exact folder + Exact filename
            strcpy(temp, contentDir);
            strcat(temp, dirs[i]);
            strcat(temp, filename);
            if (exists(temp)) { strcpy(path, temp); return; }

            // Try 2: Exact folder + All Lowercase filename
            char *p = temp + strlen(contentDir) + strlen(dirs[i]);
            while (*p) { if (*p >= 'A' && *p <= 'Z') *p += 'a' - 'A'; p++; }
            if (exists(temp)) { strcpy(path, temp); return; }

            // Try 3: Exact folder + All Uppercase filename
            p = temp + strlen(contentDir) + strlen(dirs[i]);
            strcpy(p, filename);
            while (*p) { if (*p >= 'a' && *p <= 'z') *p -= 'a' - 'A'; p++; }
            if (exists(temp)) { strcpy(path, temp); return; }

            // Try 4: Exact folder + Extension Swap (.pcx <-> .png)
            strcpy(temp, contentDir);
            strcat(temp, dirs[i]);
            strcat(temp, filename);
            char *ext = strrchr(temp, '.');
            if (ext) {
                const char *newExt = (strcasecmp(ext, ".pcx") == 0) ? ".png" : (strcasecmp(ext, ".png") == 0) ? ".pcx" : NULL;
                if (newExt) {
                    strcpy(ext, newExt);
                    if (exists(temp)) { strcpy(path, temp); return; }
                    // Try lowercase extension too
                    for (int j = 0; ext[j]; j++) if (ext[j] >= 'A' && ext[j] <= 'Z') ext[j] += 'a' - 'A';
                    if (exists(temp)) { strcpy(path, temp); return; }
                }
            }
        }
    ...
    #ifdef __RETROGO__
        return rg_storage_exists(name);
    #elif defined(_OS_LINUX)
        const char *fixed = osFixFileName(name);
        return (fixed != NULL);
    #else
    ...
    __attribute__((noinline)) static bool existsContent(const char *name) {
        ...
        bool found = exists(fileName);
        //if (found) LOG("Found asset: %s -> %s\n", name, fileName);
        return found;
    }
    ...
    inline T* read(T *&a, int count) {
        if (count) {
            a = (T*)OL_ALLOC(count * sizeof(T));
            if (a) memset(a, 0, count * sizeof(T));
...
void osDataWrite(Stream *stream, const char *dir) {
    char path[255];
        ...
        sprintf(path, "%s/%s", dir, stream->name);
        LOG("Saving to: %s\n", path);
        bool success = rg_storage_write_file(path, stream->data, stream->size, RG_FILE_ATOMIC_WRITE);
        if (success) {
            if (stream->callback)
...
void osDataRead(Stream *stream, const char *dir) {
    ...
    if (f) {
        ...
        fseek(f, 0, SEEK_SET);
        char *data = (char*)OL_ALLOC_NP(size);
        if (data) {
            fread(data, 1, size, f);
            fclose(f);
            if (stream->callback)
            ...
            fclose(f);
            LOG("! not enough RAM to read file \"%s\" (%d bytes needed)\n", stream->name, size);
            if (stream->callback)
                stream->callback(NULL, stream->userData);
        }
    } else {
