...
namespace Game {
    ...
    void cheatControl(int32 playerIndex) {
        ...
        ControlKey *playerSeq = &cheatSeq[playerIndex * MAX_CHEAT_SEQUENCE];

        for (int i = 0; i < MAX_CHEAT_SEQUENCE - 1; i++)
            playerSeq[i] = playerSeq[i + 1];
        playerSeq[MAX_CHEAT_SEQUENCE - 1] = key;

        #define CHECK_CHEAT(seq) (!memcmp(&playerSeq[MAX_CHEAT_SEQUENCE - COUNT(seq)], seq, sizeof(seq)))
    ...
    extern volatile bool sound_running;

    void startLevel(Stream *lvl) {
        LOG("Game::startLevel: inventory=%p\n", (void*)inventory);
        ...
        bool playVideo = true;
        if (loadSlot >= 0 && loadSlot < saveSlots.length)
        ...
        delete level;
        level = new Level(*lvl);
        LOG("Game::startLevel: new Level created, inventory->game=%p\n", (void*)(inventory ? inventory->game : NULL));
        ...
        LOG("Game::startLevel: calling level->init, inventory=%p\n", (void*)inventory);
        level->init(playLogo, playVideo);
        sound_active = true;

        UI::game = level;
        if (inventory)
            inventory->game = level;
...
namespace Game {
    ...
    void init(Stream *lvl) {
        LOG("Game::init(Stream): inventory=%p\n", (void*)inventory);
        loadSlot    = -1;
        nextLevel   = NULL;
        shaderCache = NULL;
        level       = NULL;

        cheatSeq = (ControlKey*)OL_ALLOC(OL_MAX_PLAYERS * MAX_CHEAT_SEQUENCE * sizeof(ControlKey));
        if (cheatSeq) memset(cheatSeq, 0, OL_MAX_PLAYERS * MAX_CHEAT_SEQUENCE * sizeof(ControlKey));
        LOG("Game::init(Stream): after cheatSeq allocation, inventory=%p\n", (void*)inventory);

        Core::init();
        LOG("Game::init(Stream): after Core::init, inventory=%p\n", (void*)inventory);
        ...
        LOG("Game::init(Stream): calling cacheRead, inventory=%p\n", (void*)inventory);
    ...
    void init(const char *lvlName = NULL) {
        ...
        LOG("Game::init: fileName='%s'\n", fileName);
        inventory = new Inventory();
        LOG("Game::init: inventory=%p\n", (void*)inventory);
    ...
    void updateTick() {
        ...
        for (int32 i = 0; i < OL_MAX_PLAYERS; i++)
        {
            if (level->players[i]) {
                cheatControl(i);
            }
