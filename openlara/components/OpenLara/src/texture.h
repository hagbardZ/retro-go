...
struct Texture : GAPI::Texture {
    ...
    Texture(int width, int height, int depth, TexFormat format, uint32 opt = 0, void *data = NULL) : GAPI::Texture(width, height, depth, format, opt) {
        #ifdef SPLIT_BY_TILE
    ...
    virtual ~Texture() {
        #ifdef __RETROGO__
            // data is freed in deinit() or manually if dynamic
        #else
            #if !defined(_GAPI_SW) && !defined(_GAPI_GU)
                #ifdef SPLIT_BY_TILE
...
#ifdef __RETROGO__
        uint8 *indices = (uint8*)rg_alloc(size, MEM_SLOW);
#else
...
#ifdef __RETROGO__
        uint16 *data = (uint16*)rg_alloc(pcx.width * pcx.height * sizeof(uint16), MEM_SLOW);
        if (data) {
            uint16 *dst = data;
            uint8  *src = indices;

            for (int i = 0; i < pcx.width * pcx.height; i++) {
                Color24 &c = palette[*src++];
                *dst++ = CONV_COLOR(c.r, c.g, c.b);
            }
        }
#else
...
#ifdef __RETROGO__
        uint8 *data = (uint8*)rg_alloc(size, MEM_SLOW);
#else
...
#ifdef __RETROGO__
        uint8 *data32 = (uint8*)rg_alloc(width * height * sizeof(Color32), MEM_SLOW);
#else
...
#ifdef __RETROGO__
                cdata = (uint8*)rg_alloc(stream.size, MEM_SLOW);
#else
...
#ifdef __RETROGO__
        uint8 *buffer = (uint8*)rg_alloc((BPL + 1) * height, MEM_SLOW);
#else
        uint8 *buffer = new uint8[(BPL + 1) * height];
#endif

        uint32 cbytes = (BPL + 1) * height; // Output size
    ...
    // apply line filters to image and convert to 16-bit
#ifdef __RETROGO__
        uint16 *data16 = (uint16*)rg_alloc(width * height * sizeof(uint16), MEM_SLOW);
        uint8 *prev = (uint8*)rg_alloc(BPL, MEM_SLOW);
        uint8 *line = (uint8*)rg_alloc(BPL, MEM_SLOW);
        
        memset(prev, 0, BPL);
        uint16 *dst16 = data16;

        for (uint32 i = 0; i < height; i++) {
            pngFilter(buffer[(BPL + 1) * i], BPP, BPL, &buffer[(BPL + 1) * i + 1], line, prev);
            
            uint8 *src = line;
            switch (colorType) {
                case 0 : // grayscale
                    for (uint32 x = 0; x < width; x++) {
                        *dst16++ = CONV_COLOR(src[0], src[0], src[0]);
                        src++;
                    }
                    break;
                case 2 : // RGB
                    for (uint32 x = 0; x < width; x++) {
                        *dst16++ = CONV_COLOR(src[0], src[1], src[2]);
                        src += 3;
                    }
                    break;
                case 3 : // indexed color
                    for (uint32 x = 0, palWord = 0, curBit = 8; x < width; x++) {
                        if (curBit > 7) {
                            curBit -= 8;
                            palWord = *src++;
                            if (x < width - 1) palWord |= *src << 8;
                        }
                        uint16 palIdx = (palWord >> (8 - bits - curBit)) & ~(0xFFFF << bits);
                        curBit += bits;
                        *dst16++ = CONV_COLOR(palette[palIdx * 3 + 0], palette[palIdx * 3 + 1], palette[palIdx * 3 + 2]);
                    }
                    break;
    ...
    static uint8* LoadRNC(Stream &stream, uint32 &width, uint32 &height) { // https://github.com/lab313ru/rnc_propack_source
        ...
        uint32 *data32;
#ifdef __RETROGO__
        data32 = (uint32*)rg_alloc(width * height * 4, MEM_SLOW);
#else
...
#ifdef __RETROGO__
        uint8 *data = (uint8*)rg_alloc(stream.size, MEM_SLOW);
        stream.raw(data, stream.size);

        uint16 *data16 = (uint16*)rg_alloc(width * height * 2, MEM_SLOW);
        uint16 *dst = data16;
        uint16 *src = (uint16*)data;
        uint16 *end = src + width * height;
    ...
    static Texture* Load(Stream &stream, bool border = true) {
        ...
        if (stream.name) {
            LOG("Texture::Load: %s (%d x %d) ptr=%p\n", stream.name, width, height, data);
        }
    ...
    // convert to POT size if NPOT isn't supported
        ...
        #ifdef __RETROGO__
            uint16 *dataPOT = (uint16*)rg_alloc(dw * dh * 2, MEM_SLOW);
            uint16 *dst = dataPOT;
            uint16 *src = (uint16*)data;

            for (uint32 j = 0; j < dh; j++)
                for (uint32 i = 0; i < dw; i++)
                    *dst++ = (i < width && j < height) ? *src++ : 0x0000;
            rg_free(data);
            data = (uint8*)dataPOT;
        #else
        ...
        #ifdef __RETROGO__
            for (uint32 y = 0; y < height; y++)
                ((uint16*)data)[y * dw] = ((uint16*)data)[y * dw + dw - 1] = 0x0000;
        #else
            for (uint32 y = 0; y < height; y++)
    ...
    #ifdef __RETROGO__
        Texture *tex = new Texture(dw, dh, 1, FMT_RGB16, 0, data);
    #else
