...
#define SND_CHANNELS_MAX    16
#define SND_OUTPUT_FREQ     22050.0f
...
namespace Sound {
    ...
    namespace Filter {
        ...
        struct Reverberation {
            ...
            void setRoomSize(const vec3 &size) {
                ...
                float k = -10.0f / (SND_OUTPUT_FREQ * rt60);
                for (int i = 0; i < MAX_FDN; i++) {
    ...
    struct Decoder {
        Stream  *stream;
        int     channels, freq, offset, phase;
        Frame   prevFrame;

        Decoder(Stream *stream, int channels, int freq) : stream(stream), channels(channels), freq(freq), offset(stream ? stream->pos : 0), phase(0) {
        ...
        int resample(Sound::Frame *frames, Sound::Frame &frame, int count) {
            if (freq == SND_OUTPUT_FREQ) {
                frames[0] = frame;
                return 1;
            }

            if (freq > SND_OUTPUT_FREQ) { // Downsampling (e.g. 44100 -> 22050)
                phase++;
                if ((phase % (int(freq / SND_OUTPUT_FREQ))) == 0) {
                    frames[0] = frame;
                    return 1;
                }
                return 0;
            ...
            switch (int(freq)) {
                case 11025 :
                    ...
                    return 2;
                default    :
                    int k = int(SND_OUTPUT_FREQ) / freq;
                    if (count < k) return 0;
    ...
    struct PCM : Decoder {
        ...
        virtual int decode(Frame *frames, int count) {
            ...
            return resample(frames, frame, count);
        }
    };
        ...
        virtual int decode(Frame *frames, int count)
        {
            ...
            if (seek % block == 0)
            {
                ...
                if (channels == 1)
                {
                    ...
                    {
                        if (count < 1) return 0;
                        // For 44k -> 22k downsampling, average the two header samples
                        frames[0].L = frames[0].R = (int(channel[0].sample1) + int(channel[0].sample2)) / 2;
                        return 1;
                    } else { // 11025
                        if (count < 2) return 0;
                        frames[0].L = frames[0].R = channel[0].sample2;
                        frames[1].L = frames[1].R = channel[0].sample2; // simple upsample
                        return 2;
                    }
                } else {
                    if (count < 1) return 0;
                    frames[0].L = (int(channel[0].sample1) + int(channel[0].sample2)) / 2;
                    frames[0].R = (int(channel[1].sample1) + int(channel[1].sample2)) / 2;
                    return 1;
                }
            } else {
                uint8 value;
                ...
                if (channels == 1)
                {
                    ...
                    {
                        if (count < 1) return 0;
                        // Average the two nibbles for 22k target
                        int s1 = channel[0].predicate(n1);
                        int s2 = channel[0].predicate(n2);
                        frames[0].L = frames[0].R = (s1 + s2) / 2;
                        return 1;
                    } else { // 11025
                        if (count < 2) return 0;
                        int s = channel[0].predicate(n1);
                        frames[0].L = frames[0].R =
                        frames[1].L = frames[1].R = s;
                        return 2;
                    }
                } else {
                    if (count < 1) return 0;
    ...
    struct IMA : Decoder { // https://wiki.multimedia.cx/?title=Microsoft_ADPCM
        ...
        virtual int decode(Frame *frames, int count) {
            ...
            int b = getSample(n & 0xF, state[1 % channels]);
            ...
            if (channels == 2) {
                ...
                // For 44k -> 22k downsampling, average the two IMA samples
                if (freq > SND_OUTPUT_FREQ) {
                    frame.L = frame.R = (a + b) / 2;
                    return resample(frames, frame, count);
                }

                frame.L = frame.R = a;
                int i = resample(frames, frame, count);
                frame.L = frame.R = b;
                return i + resample(frames + i, frame, count - i);
            }
        }
    };
#endif
    ...
    struct Sample
    {
        ...
        void setVolume(float value, float time)
        {
            ...
            if (time > 0.0f)
            {
                volumeDelta /= SND_OUTPUT_FREQ * time;
            }
        }
        ...
        // apply volume
            #define VOL_CONV(x) (1.0f - sqrtf(max(0.0f, 1.0f - x * x)))
