...
namespace TR {
    ...
    bool checkTrack(const char *pre, char *name) {
        static const char *fmt[] = { ".wav" };
        const char *lng[] = { "" }; // Skip language suffixes
            ...
            switch (version) {
                ...
                case VER_TR1_PSX : {
                    if (TR::checkWebDub(version, track)) {
                        ...
                        sprintf(title, "audio/1/track_%02d%s.wav", track, lng[Core::settings.audio.language]);
                    } else {
                        if (TR::getSubs(version, track) != STR_EMPTY) {
                            sprintf(title, "audio/1/track_%02d_EN.wav", track);
                        } else {
                    ...
                    if (Stream::existsContent(title))
                        break;
                    track = remapTrack(version, track);
                    sprintf(title, "audio/1/%03d.wav", track);
                #endif
                    break;
                }
                case VER_TR2_PC  :
    ...
    __attribute__((noinline)) const char* getGameScreen(LevelID id) {
        switch (id) {
        // TR1
            case LVL_TR1_TITLE :
    ...
    __attribute__((noinline)) const char* getGameVideo(LevelID id) {
        switch (id) {
        // TR1
            case LVL_TR1_TITLE :
                if (isGameEnded) {
